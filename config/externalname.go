...
var TerraformPluginSDKExternalNameConfigs = map[string]config.ExternalName{
	...
	"azurerm_virtual_network_dns_servers":                 config.TemplatedStringAsIdentifier("name", "/subscriptions/{{ .setup.configuration.subscription_id }}/resourceGroups/{{ .parameters.resource_group_name }}/providers/Microsoft.Network/virtualNetworks/{{ .parameters.virtual_network_name }}/dnsServers/{{ .external_name }}"),
